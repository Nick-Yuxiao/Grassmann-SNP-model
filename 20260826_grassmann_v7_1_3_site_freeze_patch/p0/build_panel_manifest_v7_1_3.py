...
def read_first_column(path: Path, header_names: set[str], allow_empty: bool = False) -> list[str]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the frozen v7.1.3 Branch-B panel manifest")
    ...
    parser.add_argument("--calibration-status-record", type=Path, required=True)
    ...
    input_paths = {
        ...
        "calibration_status_record": args.calibration_status_record,
    ...
    if spec.get("protocol_version") != "v7.1.3":
        raise SystemExit("panel spec must declare protocol_version v7.1.3")
    ...
    groups = {
        ...
        "hgdp_snpbag_calibration": read_first_column(
            args.hgdp_snpbag_calibration_samples, sample_headers, allow_empty=True
        ),
    }
    ...
    calibration_status = json.loads(args.calibration_status_record.read_text(encoding="utf-8"))
    status = calibration_status.get("status")
    if status not in {"EXACT_216_AVAILABLE", "NOT_COMPARABLE_SOURCE_MISMATCH"}:
        raise SystemExit(f"invalid calibration status: {status!r}")
    ...
    if status == "EXACT_216_AVAILABLE":
        if len(calibration) != 216 or len(cal_counts) != 54 or set(cal_counts.values()) != {4}:
            raise SystemExit("exact SNPBag calibration must contain 216 samples: 54 populations x 4")
    elif calibration:
        raise SystemExit("NOT_COMPARABLE calibration requires an empty calibration sample manifest")

    variants = read_first_column(args.variants, {"variant", "variant_id", "id", "site_id"})
    frozen_l = int(spec["site_selection"]["final_sequence_length"])
    if len(variants) != frozen_l:
        raise SystemExit(f"variant count does not match frozen L: {len(variants)} != {frozen_l}")
    ...
    payload = {
        ...
        "snpbag_calibration": {
            ...
            "reason": calibration_status.get("reason", ""),
        },
    ...
    output = args.output_dir / "PANEL_MANIFEST.v7.1.3.json"
