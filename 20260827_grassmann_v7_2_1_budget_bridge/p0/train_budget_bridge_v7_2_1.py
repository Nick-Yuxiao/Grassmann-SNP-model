...
def main() -> None:
    ...
    try:
        if args.learning_rate not in {0.0001, 0.0004}:
            raise ValueError("learning rate outside frozen bridge")
        if args.mask_seed != 92001 or args.init_seed != 82001:
            raise ValueError("seed contract mismatch")
        if args.steps != 20000 or args.warmup_steps != 500 or args.eval_interval != 250:
            raise ValueError("horizon or schedule mismatch")
        ...
        tuning_indices = [int(x) for x in (args.firewall_dir / "LR_TUNING.indices.txt").read_text().splitlines()]
        decision_indices = [int(x) for x in (args.firewall_dir / "DECISION_HOLDOUT.indices.txt").read_text().splitlines()]
        ...
        config = {
            ...
            "role": "OPTIMIZATION_BUDGET_BRIDGE_ONLY", "model": args.model,
            "mask": args.mask, "mask_seed": args.mask_seed, "init_seed": args.init_seed,
            "learning_rate": args.learning_rate, "steps": args.steps,
            "warmup_steps": args.warmup_steps, "warmup_initial_fraction": 0.10,
            "post_warmup_schedule": "CONSTANT", "eval_interval": args.eval_interval,
            "mask_bank_size": args.mask_bank_size, "tuning_validation_count": len(tuning_indices),
        ...
        curve = args.output_dir / "BUDGET_CURVE.jsonl"
        ...
        for step in range(1, args.steps + 1):
            ...
            lr = args.learning_rate * fraction if step <= args.warmup_steps else args.learning_rate
            ...
            if step % args.eval_interval == 0:
                ...
                record = {
                    ...
                    "learning_rate": lr, "gradient_norm": grad,
        ...
        checkpoint = args.output_dir / "CHECKPOINT_STEP20000.pt"
        torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "steps": 20000}, checkpoint)
        result = config | {
            "status": "PASS", "created_at_utc": datetime.now(timezone.utc).isoformat(),
            "curve_sha256": sha256(curve), "checkpoint_sha256": sha256(checkpoint),
            "final_step": 20000,
