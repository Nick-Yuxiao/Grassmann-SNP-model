...
def main() -> None:
    ...
    try:
        if args.learning_rate not in {0.0001, 0.0004}:
            raise ValueError("learning rate outside frozen extension")
        if args.mask_seed != 92001 or args.init_seed != 82001:
            raise ValueError("seed contract mismatch")
        if args.source_step != 20000 or args.target_step != 30000 or args.eval_interval != 250:
            raise ValueError("extension horizon mismatch")

        source_result_path = args.source_run_dir / "RESULT.json"
        source_curve = args.source_run_dir / "BUDGET_CURVE.jsonl"
        source_checkpoint = args.source_run_dir / "CHECKPOINT_STEP20000.pt"
        ...
        expected_source = {
            "status": "PASS", "protocol_version": "v7.2.1", "model": args.model,
        ...
        config = {
            ...
            "role": "OPTIMIZATION_BUDGET_EXTENSION_ONLY", "model": args.model,
            "mask": args.mask, "mask_seed": args.mask_seed, "init_seed": args.init_seed,
            ...
            "optimizer_resumed": True, "hgdp_used": False,
        }
        (args.output_dir / "RUN_CONFIG.json").write_text(
            json.dumps(config, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )

        curve = args.output_dir / "BUDGET_EXTENSION_CURVE.jsonl"
        ...
        target_checkpoint = args.output_dir / "CHECKPOINT_STEP30000.pt"
        torch.save({
