...
SCHEDULE="${V7_BUDGET_EXTENSION_SCHEDULE:-${ROOT}/BUDGET_EXTENSION_SCHEDULE.v7.2.3.tsv}"
OUT_PARENT="${V7_BUDGET_EXTENSION_OUT_PARENT:-${GRASS_ROOT}/v7/results/budget_extension/v7.2.3}"
GPUS=(1 3 4 5 6 7)
...
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)_budget_extension_v7_2_3_$$"
...
PY
    ...
    nice -n 10 "${V7_PY}" "${ROOT}/p0/run_budget_extension_gpu_worker_v7_2_3.py" \
      --schedule "${SCHEDULE}" --gpu "${gpu}" --data-dir "${DATA_DIR}" \
      --firewall-dir "${FIREWALL_DIR}" --source-root "${SOURCE_ROOT}" \
      --output-root "${OUT}" --trainer "${ROOT}/p0/train_budget_extension_v7_2_3.py"
...
assessment_status=0
"${V7_PY}" "${ROOT}/p0/assess_budget_extension_v7_2_3.py" \
...
(
  cd "${OUT}"
  find . -type f ! -name 'BUDGET_EXTENSION_MANIFEST.v7.2.3.sha256' -print0 \
    | sort -z | xargs -0 sha256sum > BUDGET_EXTENSION_MANIFEST.v7.2.3.sha256
)
echo "output_dir=${OUT}"
