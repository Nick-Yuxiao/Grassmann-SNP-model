...
- Verify `MANIFEST.v7.2.3.sha256`, validator, unit tests and shell syntax.
- Set `V7_BUDGET_BRIDGE_RUN_ROOT` to the immutable v7.2.1 run whose decision is
  `BUDGET_BRIDGE_EXTEND_ALL_TO_30K`.
- Confirm no duplicate v7.2.3 task exists.
...
nohup env \
  ...
  bash "$RELEASE/p0/run_budget_extension_v7_2_3_nonintrusive.sh" \
  > "$LOG" 2>&1 < /dev/null &
```
...
- 7: the primary Gate passes but at least one selected-LR cell has the prospective
  `STABLE_BUT_ACCELERATING` flag; shape review is required and no schedule is frozen.
