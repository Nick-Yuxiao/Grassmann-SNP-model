...
def main() -> None:
    ...
    try:
        if args.learning_rate != 0.0004:
            raise ValueError("only the prospectively selected 4e-4 LR is permitted")
        if args.mask_seed != 92001 or args.init_seed != 82001:
            raise ValueError("seed contract mismatch")
        if args.source_step != 30000 or args.target_step != 40000:
            raise ValueError("final diagnostic horizon mismatch")
        if args.eval_interval != 250:
            raise ValueError("evaluation interval mismatch")

        source_result_path = args.source_run_dir / "RESULT.json"
        source_curve = args.source_run_dir / "BUDGET_EXTENSION_CURVE.jsonl"
        source_checkpoint = args.source_run_dir / "CHECKPOINT_STEP30000.pt"
        if not all(path.is_file() for path in (source_result_path, source_curve, source_checkpoint)):
            raise ValueError("source v7.2.3 run is incomplete")
        source_result = json.loads(source_result_path.read_text(encoding="utf-8"))
        expected_source = {
            "status": "PASS", "protocol_version": "v7.2.3", "model": args.model,
            "mask": args.mask, "mask_seed": args.mask_seed, "init_seed": args.init_seed,
            "learning_rate": args.learning_rate, "final_step": args.source_step,
            "decision_holdout_read": False, "hgdp_used": False,
            "optimizer_resumed": True,
        }
        mismatches = {
            key: (source_result.get(key), value)
            for key, value in expected_source.items() if source_result.get(key) != value
        }
        if mismatches:
            raise ValueError(f"source-result lineage mismatch: {mismatches}")
        if sha256(source_curve) != source_result.get("curve_sha256"):
            raise ValueError("source curve hash mismatch")
        source_checkpoint_sha = sha256(source_checkpoint)
        if source_checkpoint_sha != source_result.get("checkpoint_sha256"):
            raise ValueError("source checkpoint hash mismatch")

        train = np.load(args.data_dir / "TRAIN_GT_SAMPLE_MAJOR.uint8.npy", mmap_mode="r")
        validation = np.load(
            args.data_dir / "VALIDATION_GT_SAMPLE_MAJOR.uint8.npy", mmap_mode="r"
        )
        train_pc = np.load(args.data_dir / "TRAIN_PC16.float32.npy", mmap_mode="r")
        validation_pc = np.load(
            args.data_dir / "VALIDATION_PC16.float32.npy", mmap_mode="r"
        )
        subset = np.load(
            args.data_dir / "SUBSET_100P_2247.indices.int64.npy", mmap_mode="r"
        )
        ...
        tuning_indices = [
            ...
            (args.firewall_dir / "LR_TUNING.indices.txt").read_text().splitlines()
        ]
        decision_indices = [
            int(value) for value in
            (args.firewall_dir / "DECISION_HOLDOUT.indices.txt").read_text().splitlines()
        ]
        ...
        if sorted(tuning_indices + decision_indices) != list(range(249)):
            raise ValueError("validation firewall does not partition all rows")

        blocks = ld_macroblocks(
            args.data_dir / "TRAIN_VARIANT_KEYS.txt", args.data_dir / "PCA_LD.prune.in"
        )
        ...
        if int(checkpoint.get("steps", -1)) != args.source_step:
            raise ValueError("checkpoint step mismatch")
        required_checkpoint_keys = {
            "model", "optimizer", "steps", "torch_rng_state", "cuda_rng_state_all"
        }
        if not required_checkpoint_keys <= set(checkpoint):
            raise ValueError("checkpoint lacks required model/optimizer/RNG state")
        ...
        config = {
            ...
            "role": "FINAL_OPTIMIZATION_HORIZON_DIAGNOSTIC_ONLY",
            "model": args.model, "mask": args.mask,
            "mask_seed": args.mask_seed, "init_seed": args.init_seed,
            "learning_rate": args.learning_rate, "source_step": args.source_step,
            "target_step": args.target_step, "post_resume_schedule": "CONSTANT",
            "eval_interval": args.eval_interval, "mask_bank_size": args.mask_bank_size,
            "tuning_validation_count": len(tuning_indices), "decision_holdout_read": False,
            "parameter_count": parameter_count(model),
            "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
            "data_manifest_sha256": sha256(
                args.data_dir / "PREPROCESS_MANIFEST.v7.1.10.sha256"
            ),
            "firewall_manifest_sha256": sha256(
                args.firewall_dir / "VALIDATION_FIREWALL.v7.2.0.sha256"
            ),
            "source_run_dir": str(args.source_run_dir),
            "source_result_sha256": sha256(source_result_path),
            "source_curve_sha256": sha256(source_curve),
            "source_checkpoint_sha256": source_checkpoint_sha,
            "optimizer_resumed": True, "rng_resumed": True, "hgdp_used": False,
        }
        (args.output_dir / "RUN_CONFIG.json").write_text(
            json.dumps(config, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )

        curve = args.output_dir / "FINAL_BUDGET_CURVE.jsonl"
        ...
        for step in range(args.source_step + 1, args.target_step + 1):
            for group in optimizer.param_groups:
                group["lr"] = args.learning_rate
            row = int(subset[stable_int("train_sample", args.mask_seed, step) % len(subset)])
            target = torch.from_numpy(
                np.array(train[row], copy=True, dtype=np.int64)
            ).unsqueeze(0).to(device)
            pc = torch.from_numpy(
                np.array(train_pc[row], copy=True, dtype=np.float32)
            ).unsqueeze(0).to(device)
            mask = torch.from_numpy(
                np.array(bank[(step - 1) % len(bank)], copy=True)
            ).unsqueeze(0).to(device)
            tokens = target.clone()
            tokens[mask] = 3
            ...
            if loss_value > 5.0:
                raise RuntimeError(f"training loss exceeded hard ceiling at {step}: {loss_value}")
            ...
            if step % args.eval_interval == 0:
                training_elapsed = time.perf_counter() - interval_started
                ...
                record = {
                    ...
                    "learning_rate": args.learning_rate, "gradient_norm": grad,
                    "train_masked_tokens_per_second_interval": interval_tokens / training_elapsed,
        ...
        target_checkpoint = args.output_dir / "CHECKPOINT_STEP40000.pt"
        torch.save({
            "model": model.state_dict(), "optimizer": optimizer.state_dict(),
            "steps": args.target_step, "source_checkpoint_sha256": source_checkpoint_sha,
            "torch_rng_state": torch.get_rng_state(),
            "cuda_rng_state_all": torch.cuda.get_rng_state_all(),
        }, target_checkpoint)
        result = config | {
            "status": "PASS", "created_at_utc": datetime.now(timezone.utc).isoformat(),
            "curve_sha256": sha256(curve), "checkpoint_sha256": sha256(target_checkpoint),
