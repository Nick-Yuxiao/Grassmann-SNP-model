...
## 10. v3 超参（二手来源 + 代码交叉验证）

**证据等级警告：** 以下取值由用户转述，来源为 Review Commons 作者回复 / Sciety，**本会话无法访问该来源验证**（出口策略拦截）。标注 `[二手]`。但其中数条可与 v2 原文或上游代码交叉验证，结果如下。

| 项 | 转述的 v3 取值 | 交叉验证 | 结论 |
|----|--------------|---------|------|
| embedding dim | grid {64, 128}，选 **64**（128 略好但 64 性价比高）| ✅ 与 v2 原文 "d=64" 一致（我直接读到）| **可信** |
| attention heads | **4** | ✅ 与 v2 原文 "n=4" 一致 | **可信** |
| CV | **5-fold nested**，每折 60/20/20 | ✅ 比例与 v2 原文 3:1:1 一致；折数=5 是新信息 | **可信**（折数待原文确认）|
| learning rate | grid {1e-3, 1e-4, 1e-5}，选 **1e-5** | 无法交叉验证；与代码默认 1e-3 不同 | **采纳，标二手** |
| weight decay | grid {0, 1e-5, 1e-3}，选 **1e-3** | 无法交叉验证；与代码默认一致（1e-3）| **采纳，标二手** |
| FFN | 两层 position-wise，内部 4×，**GeLU** | ❌ **与代码冲突**，见 B10 | **存疑** |
| 初始化 | SNP/gene/system 嵌入用 **uniform Xavier** | ❌ **代码中不存在**，见 B10 | **存疑** |

前三条对上，说明这份二手来源整体是可靠的；但最后两条与公开代码的冲突是实打实的，已单列为 **B10**。

### 仍未确认

`batch size`、`dropout`、`训练 epoch / 早停的正式设置`。

### 不要误用的参数（上游仓库里有两组 demo 配置，都不是论文配置）

| 来源 | hidden | lr | wd | epochs | 用途 |
|------|--------|-----|-----|--------|------|
| README 单卡示例 | 256 | 1e-3 | 1e-3 | 50 | **演示 CLI 接口** |
| `train_model.sh` | 64 | 1e-4 | 1e-4 | 21 | **合成数据 demo** |
| **论文（v2 原文 + v3 二手）** | **64** | **1e-5** | **1e-3** | 网格搜索 | ← 复现用这一列 |
